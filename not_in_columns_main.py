...
import pandas as pd
import numpy as np
from config_1 import excl_cols_report,excl_cols_upload,excl_cols_output
import openpyxl
...
def not_in_cols(st,tt,sc,tc,conn_src,conn_tar):
    ...
    dfSourceTable=pd.read_sql(src.format(st),conn)
    # print("source table>>",dfSourceTable)

    # print("targettable>>",tt)
    dfTargetTable=pd.read_sql(tgt.format(tt),conn_tar)
    # print("target table>> ",dfTargetTable)

    # print("sourcecols",dfSourceTable.columns)
    columns_not_required_src=sc.split(',')
    # print("src cols>>",columns_not_required_src)
    x=dfSourceTable.drop(columns_not_required_src,inplace=True,axis=1)
    # print("updated src table",dfSourceTable)


    # print("targetcols",dfTargetTable.columns) 
    columns_not_required_tar=tc.split(',')
    # print("tar cols>>",columns_not_required_tar)
    y=dfTargetTable.drop(columns_not_required_tar,inplace=True,axis=1)
    # print("upadted tar tabel",dfTargetTable)
...
def wrapper_excl(src_engine=None,tar_engine=None,testcase_status_list=None,testcase_status_report=None):

    # path = "C:\\Users\\muhfnu\\Desktop\\Desktop_muhfnu\\DFTE_Demo\\Table_Details\\test_file.csv" 
    path = excl_cols_upload + "\\input\\test_file_for_attribute_exclusion.csv"
    response_dict = {}
    ...
    report_wb.save(excl_cols_report+'\\'+'FinalReport.xlsx')
...
# table names from csv
    ...
    for index, Source_tables in enumerate(st):
        ...
        df = not_in_cols(st[index],tt[index],sc[index],tc[index],src_engine,tar_engine)
        if len(df.index) == 0:
...
def create_folder(table_name):
    if not(os.path.exists(os.path.join(excl_cols_output,table_name))):
        [os.makedirs(os.path.join(excl_cols_output,table_name))]
...
def write_data(table_name,file_name,df):
    ...
    header = ",".join(my_list)
    # print("Header",header)
    np.savetxt(os.path.join(excl_cols_output,table_name,file_name),df, delimiter=',',header=header,fmt='%s') 
    count=count+1
...
def create_report(testcase_status_list=None,testcase_status_report=None):
    ...
    try:
        pass
        # rep_wb = openpyxl.load_workbook(excl_cols_report + '\\' + 'FinalReport.xlsx')
        # rep_sheet = rep_wb.active
        # for tables in testcase_status_list:
        #     rep_sheet.append(tables)
        # rep_wb.save(excl_cols_report + '\\' + 'FinalReport.xlsx')
        # os.remove(excl_cols_report + '\\' + 'FinalReport.xlsx')
    except:
